          ...
          {Object.entries(value).map(([key, subValue]) => (
            <div key={key}>
              ...
              <span className="text-gray-600">
                {typeof subValue === 'object' ? renderValue(subValue) : subValue}
              </span>
            </div>
          ))}
        </div>
...
const renderUnifiedCompanyDetails = () => {
  ...
  const renderLeadRecentPosts = () => {
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {data.map((post, index) => (
          <div key={index} className="bg-gray-100 p-4 mb-4 rounded-lg shadow-sm hover:shadow-md">
            <h4 className="text-md font-bold text-gray-800 mb-2">Post {index + 1}</h4>
            ...
            <div className="text-sm">
              ...
              <span className="text-gray-600">
                Comments: {post.Metrics.Comments}, Likes: {post.Metrics.Likes}, Shares: {post.Metrics.Shares}
              </span>
            </div>
            <div className="text-sm">
        ...
        {data.map((post, index) => (
          <div key={index} className="bg-gray-100 p-4 mb-4 rounded-lg shadow-sm hover:shadow-md">
            ...
            <div className="text-sm">
              <span className="font-semibold text-gray-700">Metrics:</span>{' '}
              <span className="text-gray-600">
                Engagements: {post.Metrics.Engagements}, Shares: {post.Metrics.Shares}
              </span>
            </div>
            <div className="text-sm">
              <span className="font-semibold text-gray-700">URL:</span>{' '}
    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {data.map((project, index) => (
          <div key={index} className="bg-gray-100 p-4 mb-4 rounded-lg shadow-sm hover:shadow-md">
            <h4 className="text-md font-bold text-gray-800 mb-2">Project {index + 1}</h4>
  ...
  return (
    <div className="bg-white p-6 mt-6 rounded-lg shadow-md hover:shadow-lg transition-shadow">
      <h3 className="text-xl font-bold text-white-600 mb-4 border-b pb-2">{title}</h3>
      <div className="space-y-2">
...
function SocialMediaEngagementCard({ title, data }) {
  ...
  const renderCompanyTopPerformingPosts = () => {
    ...
    return (
      <div>
        <h5 className="text-sm font-semibold text-gray-800 mt-0 mb-2">Company Top Performing Posts</h5>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {posts.map((post, index) => (
            <div key={index} className="bg-gray-100 p-4 mb-4 rounded-lg shadow-sm hover:shadow-md">
              <h6 className="text-sm font-bold text-gray-800 mb-2">Post {index + 1}</h6>
              <div className="text-sm">
                <span className="font-semibold text-gray-700">Title:</span>{' '}
                <span className="text-gray-600">{post.Title}</span>
              </div>
              <div className="text-sm">
                <span className="font-semibold text-gray-700">Engagement Metrics:</span>{' '}
                <span className="text-gray-600">
                  Comments: {post.EngagementMetrics?.Comments ?? 'null'}, Likes: {post.EngagementMetrics?.Likes ?? 'null'}, Shares: {post.EngagementMetrics?.Shares ?? 'null'}
                </span>
              </div>
              <div className="text-sm">
                <span className="font-semibold text-gray-700">Platform:</span>{' '}
  ...
  return (
    <div className="bg-white p-6 mt-6 rounded-lg shadow-md hover:shadow-lg transition-shadow">
      <h3 className="text-xl font-bold text-white-600 mb-4 border-b pb-2">{title}</h3>
      <div className="space-y-2">
        {renderCompanyTopPerformingPosts()}
      </div>
    </div>
  );
}
      ...
      <div className="form-container">
        ...
        {leadResponse && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
            <DataCard title="Unified Lead Details" data={leadResponse.UnifiedLeadDetails} />
            <DataCard title="Unified Company Details" data={leadResponse.UnifiedCompanyDetails} />
          </div>
        )}

        {leadResponse && (
          <div className="">
            <SocialMediaEngagementCard title="Social Media Engagement" data={leadResponse.UnifiedCompanyDetails} />
            <DataCard title="Lead Recent Posts" data={leadResponse.LeadRecentPosts} />
            <DataCard title="Company Recent Posts" data={leadResponse.CompanyRecentPosts} />
            <DataCard title="Recent Projects and Works" data={leadResponse.RecentProjectsAndWork} />
        ...
        {emailResponse && (
          <div style={{ padding: '20px 40px' , margin: '20px 360px', borderRadius: '10px', backgroundColor: '#f5f5f5', border: '1px solid blue' }}>
